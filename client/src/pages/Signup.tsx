...
export default function Signup() {
  ...
  const [mobileNo, setMobileNo] = useState("");  // ✨ NEW
  ...
  const signupMutation = useMutation({
    mutationFn: async ({ username, mobileNo, password, confirmPassword }: { username: string; mobileNo: string; password: string; confirmPassword: string }) => {
      const res = await fetch(api.auth.signup.path, {
        ...
        body: JSON.stringify({ username, mobileNo, password, confirmPassword }),  // ✨ ADDED mobileNo
      });
      if (!res.ok) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!username.trim() || !mobileNo.trim() || !password.trim() || !confirmPassword.trim()) return;  // ✨ CHECK mobileNo
    signupMutation.mutate({ username: username.trim(), mobileNo: mobileNo.trim(), password, confirmPassword });  // ✨ PASS mobileNo
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl border border-slate-100 p-8 md:p-12 relative z-10 animate-in fade-in zoom-in duration-500">
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground">Join ShopKeeper Insights</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label htmlFor="mobileNo" className="text-sm font-medium text-slate-700 ml-1">
              Mobile Number
            </label>
            <input
              id="mobileNo"
          ...
          <button
            type="submit"
            disabled={!username.trim() || !password.trim() || !confirmPassword.trim() || signupMutation.isPending}
