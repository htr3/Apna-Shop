...
export function useRecordRepayment() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ customerId, amount, notes }: { customerId: number; amount: number; notes?: string }) => {
      const token = localStorage.getItem("authToken");
      const res = await fetch("/api/borrowings/payment", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`,
        },
        body: JSON.stringify({ customerId, amount: amount.toString(), notes }),
      });
